...
class InferCell(nn.Module):
  ...
  def __init__(self, genotype, C_in, C_out, stride, use_bn):
    ...
    for i in range(1, len(genotype)):
      ...
      for (op_name, op_in) in node_info:
        if op_in == 0:
          layer = OPS[op_name](C_in , C_out, stride, True, True, use_bn)
        else:
          layer = OPS[op_name](C_out, C_out,      1, True, True, use_bn)
