...
@proxy("fisher", bn=True, mode="channel")
def compute_fisher_per_weight(net, inputs, targets, bn, loss_fn, mode, split_data=1):
    ...
    grads_abs_ch = p_utils.get_layer_metric_array(net, fisher, mode)
    ...
    shapes = p_utils.get_layer_metric_array(net, lambda l: l.weight.shape[1:], mode)

    grads_abs = p_utils.reshape_elements(grads_abs_ch, shapes, device)
