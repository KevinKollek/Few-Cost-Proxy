...
def weight_measures(net, inputs, targets, bn, loss_fn=None, split_data=None):
    ...
    weight_measures_dict['magnitude_increase'] = []
    for idx, layer in enumerate(net.named_parameters()):
        if len(layer[1].size()) == 4:
            weight_measures_dict['magnitude_increase'].append(
    ...
    weight_measures_dict['magnitude_increase'] = float(sum(weight_measures_dict['magnitude_increase']))

    return weight_measures_dict['magnitude_increase']
