...
class tensorboard_logger(object):
    ...
    def __call__(self, job):
        ...
        tl.log_value('BOHB/all_results', loss * -1, self.results_logged)

        if budget not in self.incumbent_results or loss < self.incumbent_results[budget]:
            self.incumbent_results[budget] = loss
        ...
        if budget not in self.mean_results:
            self.mean_results[budget] = [loss, 1]
        else:
            self.mean_results[budget][0] += loss
