...
@proxy("nwot", bn=True)
def compute_nwot(net, inputs, targets, bn, split_data=1, loss_fn=None):
    batch_size = int(len(targets) / 2)
    ...
    net.K = np.float32(np.zeros((batch_size, batch_size)))
    ...
    x = torch.clone(inputs[:int(batch_size)])
    net(x)
    s, jc = np.linalg.slogdet(net.K)

    if np.isnan(jc) or np.isinf(jc):
        jc = -100000000
